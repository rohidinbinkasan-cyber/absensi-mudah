...
// Hanya fungsi list — tanpa auth, tanpa env var. Upload/hapus diurus backend PC
// (yang punya JWT_SECRET + BLOB_READ_WRITE_TOKEN di .env masing-masing).
...
async function presignedGetUrl(pathname) {
  const t = await issueSignedToken({ pathname, operations: ['get'], validUntil: Date.now() + 7 * 24 * 3600 * 1000 });
  const { presignedUrl } = await presignUrl(t, { operation: 'get', pathname, access: 'private' });
  return presignedUrl;
}
...
export default async function handler(req) {
  ...
  if (req.method === 'GET' && url.pathname === '/api/gallery/list') {
    try {
      ...
      for (const b of blobs || []) {
        ...
        let readable = b.url;
        try { readable = await presignedGetUrl(b.pathname); } catch (e) { /* pakai URL asli bila gagal */ }
        items.push({
          id: b.pathname,
          pathname: b.pathname,
          url: readable,
      ...
      return new Response(JSON.stringify({ error: 'Gagal memuat galeri: ' + (e.message || e) }), {
        status: 500,
